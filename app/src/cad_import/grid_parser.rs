...
pub fn parse_grids(geom: &RawGeometry) -> GridSystem {
    ...
    log::info!("Grid detection: found {} X label candidates, {} Y label candidates",
        x_labels.len(), y_labels.len());
    for (name, x, y) in &x_labels {
        log::info!("  X label '{}' at ({:.0}, {:.0})", name, x, y);
    }
    ...
    log::info!("After dedup: {} X labels, {} Y labels", x_labels.len(), y_labels.len());
    ...
    if system.x_grids.len() < 2 || system.y_grids.len() < 2 {
        log::info!("Grid: TEXT-only detection incomplete, trying TEXT+LINE proximity");
    ...
    log::info!("Grid: collected {} dimension values: {:?}", dim_values.len(),
        dim_values.iter().map(|v| format!("{:.0}", v)).collect::<Vec<_>>());

    if system.x_grids.len() < 2 && !dim_values.is_empty() {
        log::info!("Grid: using DIMENSION chain to build X grids");
    ...
    if system.x_grids.len() >= 2 && system.y_grids.is_empty() {
        ...
        log::info!("Grid: added default Y grids at 0 and {:.0}", y_span);
    }
    if system.y_grids.len() >= 2 && system.x_grids.is_empty() {
        ...
        log::info!("Grid: added default X grids at 0 and {:.0}", x_span);
    }

    log::info!("Grid final result: {} X grids, {} Y grids", system.x_grids.len(), system.y_grids.len());
    for g in &system.x_grids {
        log::info!("  X grid '{}' at {:.0}", g.name, g.position);
    }
    for g in &system.y_grids {
        log::info!("  Y grid '{}' at {:.0}", g.name, g.position);
    }
