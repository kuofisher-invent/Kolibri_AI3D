...
| `import_file` | 匯入檔案到場景（OBJ/STL/DXF） | `path` |
| `export_scene` | 匯出場景（OBJ/STL/DXF/glTF） | `path` |
| `batch_create` | 批量建立多個物件 | `objects` (array) |
| `set_object_property` | 設定物件屬性（name/tag/visible/locked/roughness/metallic） | `id` |
| `measure_object` | 測量物件面積和體積 | `id` |
| `measure_distance` | 測量兩物件中心距離 | `id_a`, `id_b` |
| `align_objects` | 對齊多個物件 | `ids`, `mode` |
| `undo` | 撤銷上一步 | — |
| `redo` | 重做 | — |
| `shutdown` | 關閉應用程式 | — |
