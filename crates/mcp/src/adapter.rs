...
impl KolibriAdapter {
    ...
    pub fn tool_definitions(&self) -> Vec<ToolDef> {
        vec![
            ...
            // ── 2D Drafting Tools ──
            ToolDef { name: "draft_add_line".into(), description: "2D: 繪製直線 (mm)".into(),
                input_schema: json!({"type":"object","required":["p1","p2"],"properties":{"p1":{"type":"array","items":{"type":"number"}},"p2":{"type":"array","items":{"type":"number"}}}}),
            },
            ToolDef { name: "draft_add_circle".into(), description: "2D: 繪製圓".into(),
                input_schema: json!({"type":"object","required":["center","radius"],"properties":{"center":{"type":"array","items":{"type":"number"}},"radius":{"type":"number"}}}),
            },
            ToolDef { name: "draft_add_arc".into(), description: "2D: 繪製弧（角度 radians）".into(),
                input_schema: json!({"type":"object","required":["center","radius","start_angle","end_angle"],"properties":{"center":{"type":"array","items":{"type":"number"}},"radius":{"type":"number"},"start_angle":{"type":"number"},"end_angle":{"type":"number"}}}),
            },
            ToolDef { name: "draft_add_rectangle".into(), description: "2D: 繪製矩形".into(),
                input_schema: json!({"type":"object","required":["p1","p2"],"properties":{"p1":{"type":"array","items":{"type":"number"}},"p2":{"type":"array","items":{"type":"number"}}}}),
            },
            ToolDef { name: "draft_add_polyline".into(), description: "2D: 繪製多段線".into(),
                input_schema: json!({"type":"object","required":["points"],"properties":{"points":{"type":"array","items":{"type":"array","items":{"type":"number"}}},"closed":{"type":"boolean","default":false}}}),
            },
            ToolDef { name: "draft_add_text".into(), description: "2D: 放置文字".into(),
                input_schema: json!({"type":"object","required":["position","content"],"properties":{"position":{"type":"array","items":{"type":"number"}},"content":{"type":"string"},"height":{"type":"number","default":5.0}}}),
            },
            ToolDef { name: "draft_add_dim_linear".into(), description: "2D: 線性標註".into(),
                input_schema: json!({"type":"object","required":["p1","p2"],"properties":{"p1":{"type":"array","items":{"type":"number"}},"p2":{"type":"array","items":{"type":"number"}},"offset":{"type":"number","default":10.0}}}),
            },
            ToolDef { name: "draft_delete".into(), description: "2D: 刪除圖元".into(),
                input_schema: json!({"type":"object","required":["id"],"properties":{"id":{"type":"integer"}}}),
            },
            ToolDef { name: "draft_clear".into(), description: "2D: 清空所有圖元".into(),
                input_schema: json!({"type":"object","properties":{}}),
            },
            ToolDef { name: "draft_list".into(), description: "2D: 列出所有圖元".into(),
                input_schema: json!({"type":"object","properties":{}}),
            },
            ToolDef { name: "draft_get_entity".into(), description: "2D: 取得圖元詳細資訊".into(),
                input_schema: json!({"type":"object","required":["id"],"properties":{"id":{"type":"integer"}}}),
            },
            ToolDef { name: "draft_set_tool".into(), description: "2D: 切換工具 (select/line/circle/arc/rectangle/polyline/text/move/rotate/mirror/trim/offset/dim_linear/erase/copy)".into(),
                input_schema: json!({"type":"object","required":["tool"],"properties":{"tool":{"type":"string"}}}),
            },
            ToolDef { name: "draft_select".into(), description: "2D: 選取圖元".into(),
                input_schema: json!({"type":"object","required":["ids"],"properties":{"ids":{"type":"array","items":{"type":"integer"}}}}),
            },
        ]
    }
